...
car à notre connaissance le guide n'a pas encore été traduit. 

Plus précisément ce document entend progressivement prendre la place d'un support
de référence dans le cadre du parcours de formation "R" et pourrait-être un
outil d'acculturation à faire vivre pour établir un lien avec la communauté
des utilisateurs concernés.

Au-delà du manuel de référence dont le volume est imposant, il existe une 
...
Pour utiliser R Markdown, il faut que les packages `rmarkdown` et `markdown` 
soient installés :
...
> ASK: Concernant cette partie, je ne suis pas sûr de ce que l'on veut
> passer comme information.
> En réalité, pour faire du R Markdown le paquet `rmarkdown` est le 
> seul strictement nécessaire (dans les faits il repose sur knitr et
> quelques autres) mais justement le paquet markdown n'est q'un plugin
> possible de knitr pour un usage aujourd'hui un peu révolu).
> Ce qui reste vrai, c'est que RStudio oblige à l'installation de tout 
> un tas de paquet pour faire tourner son environnement de rendu et là 
> figurent bien souvent `markdown` et tout un tas d'autres comme shiny
> qui ne d'ailleurs pas forcément des plus légers, mais en l'occurence
> il n'y a pas trop à réfléchier puisqu'il faut se laisser guider par 
> l'interface et la laisser faire.
> --- LBn
...
étapes 2 à 4 peuvent se faire dans n'importe quel ordre :
  ...
  6. Publier le document (particulier)
...
d'initialiser le fichier qui va être créé autrement dit afin de
remplir quelques champs fortement recommandé par le processus global,
voire imposés.
...
Il est parfois appelé le Header ou le YAML. Il sert à définir les
...
> ASK: je mettrai bien le passage sur les options des output dans le $ 
> dédié à la fin
> --- CCn
...
Les bases du langage sont très bien expliquées dans 
...
> ASK: comment faire un renvoi vers un paragraphe du document ? 
> ne fonctionne pas
> --- CCn

> ANS: en fait cela fonctionne bien mais sous condiitions
> sera ma première réponse pour éviter de rentrer trop dans les détails
> mais il y a une configuration pour que cela marche à coup sûr localement
> avec un langage à caractère acentué comme le français
> elle nécessite l'extension pandoc (md_extensions) ascii_identifiers
> --- LBn

<!-- `[Retour vers le paragraphe](#Les éléments texte)`  -->

`` [Retour vers le paragraphe](#les-elements-texte) ``
...
> ASK: Concernant des éléments encore plus génériques (hors titres)
> Le plus simple c'est de reccourrir en html à un hack en mettant une ancre
> html, en gros c'est aussi un des problème qu'adressait le démarrage de bookdown
> ne fonctionne pas
> En clair une solution possible
> poser une ancre ``<a name="id-unique"/>``
> et plus loin dans le texte y faire référence du genre ``[vers mon ancre](#id-unique)``
> --- LBn
...
![](assets/img/icone_ajout_chunk.png) 
...
r. C’est dans cette accolade, après la lettre r (il ne faut pas
l’enlever) que les options vont pouvoir être passées. Elles permettent
...
Il est possible définir des options globales qui seront appliquées à
...
régler les options par défaut de tous les chunks. Par exemple, on va
...
Pour cela, on utilise, dans un chunk, différentes fonctions R comme
`plot` ou `kable`.
...
> ASK: comment afficher les côtes pour r numero?
> --- CCn

> ANS: cf. exemple 
> `` `r knitr::inline_expr("numero")` ``
> la doc officielle 
> https://bookdown.org/yihui/rmarkdown-cookbook/verbatim-code-chunks.html#show-a-verbatim-inline-expression
> [ndr] le premier hack ne semble pas tellement fonctionner pour moi aujourd'hui 
> et de manière générale les verbatims sont encore aussi des choses un peu 
> subtiles à prendre en compte par l'EDI d'où le hack `embed` que j'utilise plus
> bas
> --- LBn

Je suis actuellement en train de me former au module `` `r knitr::inline_expr("numero")` ``.

Ce qui donne dans le document:

Je suis actuellement en train de me former au module  `r numero`.



Vous pouvez aussi affecter des valeurs par défaut à l'ensemble de vos
chunks avec la fonction `knitr::opts_chunk$set()` à intégrer dans
votre premier chunk :
...
<a name="TD-3"/>TD.3
  ...
  - Ajouter un chunk créant une table, qui sera traité mais non
    affiché dans le document final *quel traitement de données?*
...
> ASK: indentation auto?
> --- CCn

#### Affichages sur plusieurs colonnes

Vous aurez besoin souvent de pouvoir par exemple définir un document
en plusieurs colonne, avec par exemple une illustration sur une
...
modifiant l'option de l'en-tête ou en cliquant sur la flèche vers le
bas, du bouton knit.
...
> ASK: est ce qu'on parle des présentations?
> --- CCn
...
format qui a le plus de possibilités parmi tous les formats.
...
# lbn-fashion--start
install_path <- file.path(Sys.getenv("LOCALAPPDATA"),'programs','RDIR','TinyTex',fsep = "\\")
# lbn-fashion--end
...
## Documentation (comment aller plus loin ?)

### Options possibles par type de sortie

L'ensemble des options est listé page 5 du 
[document de référence Rmarkdown](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf)  

Each output format is often accompanied with several format
options. All these options are documented on the R package help
pages. For example, you can type ?rmarkdown::html_document in R to
open the help page of the html_document format. When you want to use
certain options, you have to translate the values from R to YAML The
translation is often straightforward. Remember that R’s TRUE, FALSE,
and NULL are true, false, and null, respectively, in YAML. Character
strings in YAML often do not require the quotes (e.g., dev: 'svg' and
dev: svg are the same), unless they contain special characters, such
as the colon :.
