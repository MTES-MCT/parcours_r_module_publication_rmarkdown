...
Pour les partager en interne, les déposer sur un serveur bureautique suffit. 
En revanche, pour les partager avec l'extérieur, il est souvent nécessaire d'héberger les documents sur un serveur dédié. 
...
Il faut au préalable avoir la validation d'une nouvelle url pour votre organisme par de la direction de la communication. 
...
Github et Gitlab sont deux plateformes web concurrentes, dites forge de développement, utilisées pour le travail collaboratif sur le code. 
Pour les utiliser, il faut versionner ses projets R avec git. 
...
Chaque service du pôle ministériel peut disposer un dépôt gitlab-forge après avoir formulé une demande officielle, cf fiche spote https://spote.developpement-durable.gouv.fr/offre/gitlab-forge-logicielle-et-livraison-continu. 

Chaque agent disposant d'un compte CERBERE peut rejoindre un groupe gitlab-forge ou y disposer d'un dépôt personnel. 

Si besoin de collaborer avec des personnes qui n'ont pas de compte CERBERE, gitlab.com offre une version freemium accessible de tous. A la fin de la collaboration, on peut facilement réimporter le travail sur gitlab-forge. 

### Comment on fait ?

Pour configurer git et RStudio, suivre les fiches tutorielles : 
https://dreal-pdl.gitlab-pages.din.developpement-durable.gouv.fr/csd/geodata4apps.fiches/decouverte.html
...
1. dans votre projet RStudio, placez les fichiers html (et leurs éventuelles dépendances) que vous voulez publier dans une dossier nommé 'public'.
...
2. Acceptez de redémarrer R pour que les fonctionnalités git s'activent dans votre projet Rstudio (nouveau panneau)  

A cette étape, votre projet a une version, prête à être partagée sur une forge git, reste à indiquer où. 

1. Depuis votre dépôt gitlab-forge, créer un nouveau projet **vide** (décocher "read-me").   
2. Copier l'url https de ce projet    
![](assets/img/GitLab1.png)

3. Retourner dans RStudio pour connecter votre projet R avec ce projet git, lancer dans la console :
usethis::use_git_remote(url = "https://gitlab-forge.din.developpement-durable.gouv.fr/groupe/projet.git")

4. Dernière étape pour partager le code : connexion des branches et premier 'push', dans le **terminal de commande**    
...
![](assets/img/gitlab3.png)
- et un fichier .gitlab-ci.yml à la racine du projet.  
Voici un exemple de script à placer dans votre projet sous le nom `.gitlab-ci.yml`
