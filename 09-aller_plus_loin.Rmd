...
- on souhaite tirer partie de documents bureautiques modèles, conformes à une charte graphique que l'on souhaite respecter, 
- on souhaite faire éditer le document produit par des non-Ristes.

Cela peut s'avérer nécessaire pour faire relire/valider le document avant sa publication, mais attention ! il faudra dans un second temps reporter dans Rmarkdown les modifications opérées en dehors, pour tirer pleinement partie de la reproductibilité offerte par Rmarkdown. 
...
- includes : liste nommée de contenu supplémentaire à inclure dans le document comme une entête ou un bas de page (A créer à l'aide de la [fonction `includes()`](https://pkgs.rstudio.com/rmarkdown/reference/includes.html)).     
...
La gestion des **tableaux** est parfois moins souple qu'en HTML/PDF, mais on peut obtenir quelque chose de propres.
...
```markdown
## Exemple d'onglets {.tabset .tabset-fade .tabset-pills}
```
