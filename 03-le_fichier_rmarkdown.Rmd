...
L'en-tête s'écrit au format [YAML](https://fr.wikipedia.org/wiki/YAML) c'est un format ayant pour objectif de représenter des informations plus élaborées que le simple CSV en gardant cependant une lisibilité presque comparable.
...
Par défaut, dans un script Rmarkdown, le répertoire de travail (donc l'adresse à partir de laquelle commence le chemin de l'image) se situe au niveau du dossier dans lequel est enregistré le fichier .Rmd, même si vous travaillez dans le cadre d'un projet. 
Se référer à la partie dédiée \@ref(piege) pour s'en dépatouiller.
:::
...
![](assets/img/enregistrer_encodage.png)
  ...
  Remarquer la case à cocher au niveau de la fenêtre de dialogue "Réouvrir avec encodage...",
