...
faciliter la gestion d'annexes, pour citer les fonctionnalités par
lesquelles l'extension bookdown enrichit significativement
...
produit peut-être utilisé à toute fin qui consiste à agréger un
...
communes à tous ces format et en préservant la même syntaxe à travers
...
répondre à une première question pratique : comment puis-je créer mon
...
chapitres, chacun de ces chapitres consiste en un fichier R Markdown,
portant l'extension `.Rmd`. Ces fichiers R Markdown doivent débuter
*immédiatement* par le titre du chapitre en utilisant le marquage
...
:::
  ...
  la première  pour ouvrir un fichier avec l'encodage qui vous
  convient 
  
  File $\Rightarrow$ Reopen file with encoding...
  
  Fichier $\Rightarrow$ Réouvrir avec encodage...
  
  et son pendant pour enregistrer un fichier au bon encodage :
  
  File $\Rightarrow$ Save with encoding...
  
  Fichier $\Rightarrow$ Sauvegarder avec l'encodage...
  
  Concernant la première fonctionnalité "Réouvrir avec encodage...",
  dans la fenêtre de dialogue n'hésitez surtout pas à séléectionner
...
Allons maintenant au-délà de l'interface RStudio et du système
relativement mécanique de ce que nous appelons le
clique-bouton. Non que le clique-bouton n'ai pas sa raison d'être,
...
désactivez-en le clavier tactile ... ... vous n'êtez évidemment pas
...
R), quand la seconde n'exige même pas information car par défaut elle
prendra le répertoire en cours pour tenter d'exploiter tous les
fichiers `.Rmd` disponible afin d'en constituer un *livre*. Le
