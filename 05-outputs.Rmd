...
Comme souvent avec R, il existe de nombreuses extensions qui complètent les possibilités offertes par `{rmarkdown}`. Elles sont listées sur <https://rmarkdown.rstudio.com/formats.html/> et, pour les plus connues d'entre elles, présentées dans le [guide Rmarkdown au niveaux des chapitres 5 à 10](https://bookdown.org/yihui/rmarkdown/dashboards.html).
...
Pour cela il faudra utiliser les options de positionnement dans les chunks (par exemple `fig.pos="h"`) Les [valeurs possibles pour `fig.pos`](https://www.overleaf.com/learn/latex/Positioning_images_and_tables#The_figure_environment) sont détaillées dans la documentation de LaTeX.
