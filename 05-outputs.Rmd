...
Pour déclarer la langue du document HTML, mettre dans l'entête YAML :  
...
Cela génère de nouvelles entrées dans le menu 'knit', auxquelles ont peut accéder en cliquant sur le petit triangle noir. 
En clic bouton on ne peut "rendre" qu'un format à la fois. 
En ligne de commande, on peut demander à produire toutes les sorties. 

```r
library(rmarkdown)
render(input = "nom_de_mon_script.Rmd", output_format = "all")
```
