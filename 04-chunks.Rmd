# Intégrer du code dans les Rmd - Les *chunks* {#chunks}
...
Au début de chaque chunk se trouve une accolade contenant la lettre r^[C'est pour indiquer à knitr que le code à exécuter est en langage R, cf https://bookdown.org/yihui/rmarkdown/language-engines.html pour connaître la liste des langages possibles]. 
C'est dans cette accolade, après la lettre r (qu'il ne faut pas enlever) que les options vont pouvoir être passées. 
...
Cela rend ses résultats sont utilisables par d'autres chunks.
...
Dans cet exemple, on demande à ne pas faire apparaître les messages ou les warnings parfois générés lors de l'exécution de code, et qui viendrait polluer le rapport mis en forme.


## Utiliser des sorties R 
...
Enfin, il est possible d'**insérer la valeur d'objets R** (variable, liste, résultat de calcul simple...) dans du texte. Pour cela il faut inclure l'objet R entre `` `r knitr::inline_expr("...code...")` ``. 
